...
ap.add_argument('-s', type=str, nargs=1, help='Sorted sam file (required)', required=True)
ap.add_argument('-f', type=str, nargs=1, help='Genome fasta file (required)', required=True)
ap.add_argument('-p', type=str, nargs=1, help='Output prefix (required)', required=True)
ap.add_argument('-x', type=str, nargs=1, help='Capped flag: capped or no_cap')
ap.add_argument('-e', type=str, nargs=1, help='Collapse exon ends flag: common_ends or longest_ends (default common_ends)', default="common_ends")
ap.add_argument('-c', type=str, nargs=1, help='Coverage (default 99)', default=99)
ap.add_argument('-i', type=str, nargs=1, help='Identity (default 85)', default=85)
ap.add_argument('-icm', type=str, nargs=1, help='Identity calculation method (default ident_cov for including coverage) (alternate is ident_map for excluding hard and soft clipping)', default="ident_cov")
ap.add_argument('-a', type=str, nargs=1, help='5 prime threshold (default 10)', default=10)
ap.add_argument('-m', type=str, nargs=1, help='Exon/Splice junction threshold (default 10)', default=10)
ap.add_argument('-z', type=str, nargs=1, help='3 prime threshold (default 10)', default=10)
ap.add_argument('-d', type=str, nargs=1, help='Flag for merging duplicate transcript groups (default is merge_dup will merge duplicates ,no_merge quits when duplicates are found)', default="merge_dup")
ap.add_argument('-sj', type=str, nargs=1, help='Use error threshold to prioritize the use of splice junction information from collapsing transcripts(default no_priority, activate with sj_priority)', default="no_priority")
ap.add_argument('-sjt', type=str, nargs=1, help='Threshold for detecting errors near splice junctions (default is 10bp)', default=10)
ap.add_argument('-lde', type=str, nargs=1, help='Threshold for amount of local density error near splice junctions that is allowed (default is 1000 errors which practically means no threshold is applied)', default=1000)
ap.add_argument('-ses', type=str, nargs=1, help='Simple error symbol. Use this to pick the symbol used to represent matches in the simple error string for LDE output.', default="_")

ap.add_argument('-b', type=str, nargs=1, help='Use BAM instead of SAM', default="SAM")
ap.add_argument('-log', type=str, nargs=1, help='Turns off log output to screen of collapsing process. (default on, use log_off to turn off)', default="log_on")
ap.add_argument('-v', type=str, nargs=1, help='Prints out version date and exits.')

ap.add_argument('-rm', type=str, nargs=1, help='Run mode allows you to use original or low_mem mode, default is original', default="original")
ap.add_argument('-vc', type=str, nargs=1, help='Variation covwerage threshold: Default 5 reads', default=5)
